...
def register_navigation_callbacks(app):
    ...
    @app.callback(
        ...
        [
            ...
            # --- Filters that should reset the page ---
            # Social (RangeSlider -> value)
    ...
    def update_page(
        ...
        if not ctx.triggered:
            return 0  # default to first page on initial load

        # ID of the component that triggered the callback
        triggered_id = ctx.triggered[0]['prop_id'].split('.')[0]

        # If tab changed or any filter changed, reset to page 0
        if triggered_id not in ('prev_page', 'next_page'):
            return 0

        # Handle pagination buttons (keep behavior limited to social tab)
    ...
    clientside_callback(
        ...
        """,
        Output('_', 'children'),  # dummy output, ensure a Div(id="_") exists in layout
