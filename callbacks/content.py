...
from util.plot_us_ad_map import plot_us_ad_map


def register_content_callbacks(app, data, df_ads, codebook, green_brown_colors, classification_labels):
    """
    Register callbacks for the content section of the dashboard.

    Notes:
    - df_ads is a SEPARATE dataframe (ads metadata) used ONLY for the US map.
      It is NOT filtered by any social/analytics sidebar filters, except the
      'analytics_map_metric' control (impressions/spend).
    """
    ...
    @app.callback(
        ...
        [
            ...
            # NEW controls
            Input("totals_chart_type", "value"),     # 'pie' or 'bar' for overview totals
            Input("analytics_map_metric", "value"),  # 'impressions' or 'spend' for ads map
        ],
    )
    def render_tab(
        tab_name,
        ...
        # NEW
        totals_chart_type, an_map_metric
    ):
        filtered_data = data.copy()

        # --- helper: first existing col name from candidates ---
        ...
        # ------------ pick context (social vs analytics) ------------
        if tab_name == "social_media":
            # Years
            if sm_years and len(sm_years) == 2:
                y0, y1 = int(sm_years[0]), int(sm_years[1])
                start_date, end_date = f"{y0}-01-01", f"{y1}-12-31"   # no trailing comma!
            else:
                start_date = end_date = None

            companies, entities, platforms, classifications = (
                sm_companies or [], sm_entities or [], sm_platforms or [], sm_classifs or []
            )
            uniqueness = sm_uniqueness or "all"

            # Social subcategory filters
            sm_fossil_subcategories = sm_fossil_subcategories or []
            sm_green_subcategories = sm_green_subcategories or []
            subcategory_filters = {
            ...
            companies, entities, platforms = (
                an_companies or [], an_entities or [], an_platforms or []
            )
            uniqueness = an_uniqueness or "all"
            keyword_search = None  # no keyword filter in analytics

            an_fossil_subcategories = an_fossil_subcategories or []
            an_green_subcategories = an_green_subcategories or []
            subcategory_filters = {
        ...
        # ------------ robust text columns for dedupe/search ------------
        ...
        # Date filter
        if start_date and end_date and "attributes.published_at" in filtered_data.columns:
            col_date = "attributes.published_at"
            if not pd.api.types.is_datetime64_any_dtype(filtered_data[col_date]):
                filtered_data[col_date] = pd.to_datetime(
                    filtered_data[col_date], errors="coerce"
                )
        ...
        # Company / entity / platform
        if companies:
            filtered_data = filtered_data[filtered_data["company"].isin(companies)]

        if entities and "attributes.search_data_fields.channel_data.channel_name" in filtered_data.columns:
            filtered_data = filtered_data[
                filtered_data[
        ...
        if platforms and "attributes.search_data_fields.platform_name" in filtered_data.columns:
            filtered_data = filtered_data[
                filtered_data["attributes.search_data_fields.platform_name"].isin(
                    platforms
                )
            ]
        ...
        if tab_name == "social_media":
            if sm_classifs:
                filtered_data = filtered_data[
                    filtered_data["green_brown"].isin(sm_classifs)
                ]

            posts_per_page = 20
            if view_toggle == "all_posts":
                start = (current_page or 0) * posts_per_page
                ...
                # two columns
                mid = (len(all_posts) + 1) // 2
                left_posts = all_posts[:mid]
                right_posts = all_posts[mid:]
                ...
                sidebar_badge = html.Div(
                    ["Showing ", html.Strong(f"{len(filtered_data)}"), " posts"],
                    className="post-count",
                )
                return content_div, sidebar_badge, ""

            # comparison (kept intact but unchanged)
            elif view_toggle == "compare_posts":
                left_data = filtered_data[filtered_data["green_brown"] == (left_view or "green")]
                right_data = filtered_data[filtered_data["green_brown"] == (right_view or "brown")]
                        ...
                        html.Div(
                            [
                                html.Div(
                                    [
                                        html.H3(
                                            f"{classification_labels[left_view]} Posts",
                                ...
                                html.Div(
                                    [
                                        html.H3(
                                            f"{classification_labels[right_view]} Posts",
                                            className="comparison-title",
                                            style={
                                                "margin": "0 0 12px 0",
                                                "textAlign": "center",
        ...
        # ===================== ANALYTICS TAB =====================
            ...
            chosen_totals = (totals_chart_type or "pie")

            overview_fig = plot_overview(
                filtered_data, codebook, green_brown_colors, totals_chart_type=chosen_totals
            )
            raw_greenwashing_fig = plot_combined_greenwashing_scores(filtered_data)
            green_share_fig = plot_green_share(filtered_data)

            # US ads map: built from df_ads ONLY (unfiltered), controlled by analytics_map_metric
            metric = an_map_metric if an_map_metric in ("impressions", "spend") else "impressions"
            map_fig = plot_us_ad_map(df_ads, metric=metric, state_col="region")
            ...
            content_div = html.Div(
                [
                    ...
                    html.Div(
                        [
                            html.H2("Geography of Ads (US States)", className="analytics-header"),
                            html.P(
                                "Aggregated by state using median estimates per ad, summed per state.",
                                className="analytics-description",
                            ),
                            html.Div(
                                dcc.Graph(
            ...
            analytics_badge = html.Div(
                ["Analysis based on ", html.Strong(f"{len(filtered_data)}"), " posts"], className="post-count"
            )
        ...
        # ===================== ABOUT TAB =====================
            ...
            content_div = html.Div(
                [
                    html.Div(
                        [
                            html.P([html.Strong("What is CLAIMS?")], style={"font-size": "20px", "margin-bottom": "8px"}),
                            html.P(
                            ...
                            html.P([html.Strong("How does CLAIMS work?")], style={"font-size": "20px", "margin-bottom": "8px"}),
                            html.P([html.Strong("Model")], style={"font-size": "14px", "margin-bottom": "8px"}),
                            html.P(
                            ...
                            html.P([html.Strong("Typology")], style={"font-size": "14px", "margin-bottom": "8px"}),
                            html.P(
                            ...
                            html.P([html.Strong("Glossary")], style={"font-size": "14px", "margin-bottom": "16px"}),
                            html.Ul(
                                [
                                    html.Li(
                                        [
